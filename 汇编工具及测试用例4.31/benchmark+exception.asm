...
# 保护EPC， STATUS
mfc0 $k0, $14       # EPC, Get restart address
sw $k0, 124($0)     # Save epc in memory    固定位置，两级中断以上有问题！！！
mfc0 $s3, $12       # Get Status
sw $s3, 120($0)     # Save Status
mfc0 $k0, $13       # Get Cause
addi $k1, $0, 0x0000ff00
and $k0, $k0, $k1
srl $k0, $k0, 8
addi $s0, $0, 0
...
sub $t0,$t0,$t1
bne $t0,$0,exception_right
srl $s0, $s0, 28  
#######################################

sub $s6,$s6,$t1
beq $s6,$0, exception_exit
j exception_loop
...
# 恢复EPC， STATUS
lw $k0, 124($0)     # Save epc in memory    固定位置，两级中断以上有问题！！！
mtc0 $k0, $14       # EPC, Get restart address
lw $s3, 120($0)     # Save Status
mtc0 $s3, $12       # Get Status
